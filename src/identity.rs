...
use windows::Win32::Devices::Display::{
    ...
    DISPLAYCONFIG_PATH_INFO, DISPLAYCONFIG_SOURCE_DEVICE_NAME, DISPLAYCONFIG_TARGET_DEVICE_NAME, QDC_ONLY_ACTIVE_PATHS,
};
...
pub fn parse_edid(bytes: &[u8]) -> Option<Edid> {
    ...
    let manufacturer: String =
        [(mfg >> 10) & 0x1F, (mfg >> 5) & 0x1F, mfg & 0x1F].iter().map(|&v| letter(v)).collect();
    ...
    let serial_str = [54usize, 72, 90, 108].iter().find_map(|&off| {
        ...
        (d[0] == 0 && d[1] == 0 && d[2] == 0 && d[3] == 0xFF).then(|| {
            let text: String =
                d[5..18].iter().take_while(|&&b| b != 0x0A && b != 0).map(|&b| b as char).collect();
            text.trim().to_string()
        })
    });
