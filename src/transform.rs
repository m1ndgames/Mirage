...
/// How a source region is fitted into its target rectangle. `Fit::Fit` is the
/// name users see in config.toml, hence the lint exception.
...
pub fn layout(t: &Transform, src: Rect, src_size: (i32, i32), dst: Rect) -> Layout {
    ...
    let (disp_w, disp_h) = if quarter_turn {
        (src.h as f64, src.w as f64)
    } else {
...
mod tests {
    ...
    fn flips_mirror_the_axes() {
        ...
        assert_eq!(
            apply(m, 0.0, 0.0),
            (0.5, 0.0),
            "left edge samples the right edge of the crop"
        );
    ...
    fn rotation_90_clockwise_puts_source_top_left_at_display_top_right() {
        ...
        assert_eq!(
            apply(m, 0.0, 0.0),
            (0.0, 0.5),
            "display top-left shows source bottom-left"
        );
        assert_eq!(
            apply(m, 1.0, 1.0),
            (0.5, 0.0),
            "display bottom-right shows source top-right"
        );
    }
    ...
    fn rotation_180_and_270() {
        ...
        assert_eq!(
            apply(m, 0.0, 0.0),
            (0.5, 0.0),
            "display top-left shows source top-right"
        );
    }
    ...
    fn enums_serialise_lowercase() {
        ...
        .unwrap();
        assert!(
            text.contains("fit = \"fill\"") && text.contains("filter = \"nearest\""),
            "{text}"
        );
